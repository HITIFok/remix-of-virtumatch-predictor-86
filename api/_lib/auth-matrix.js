...
export const API_AUTH_MATRIX = Object.freeze([
  ...
  },
  {
    ...
    methods: ['GET', 'POST'],
    authRequired: false,
    authTypes: [AUTH_TYPES.NONE, AUTH_TYPES.USER_BEARER],
    authFallback: 'request/verify/register/latest-apk are public; refresh-token & delete-account require User Bearer',
    rateLimited: true,
    rateLimit: '3 req/15min per email + 3 req/15min per IP (request); 10 req/60min per IP (refresh-token); 3 req/60min per IP (delete-account)',
    corsEnabled: true,
    isPublic: true,
    riskLevel: 'LOW',
    notes: 'Email enumeration protection; magic links are single-use, 15-min expiry; register action (formerly device-register) consolidated here; refresh-token action rotates Bearer token; delete-account action performs GDPR Art.17 cascade deletion',
  },
  {
  ...
  },
  {
    ...
    methods: ['GET', 'POST'],
    authRequired: true,
    authTypes: [AUTH_TYPES.USER_BEARER, AUTH_TYPES.DEVICE_HMAC, AUTH_TYPES.CRON_KEY, AUTH_TYPES.NONE],
    authFallback: 'Cron mode via x-cron-key header; client mode via Bearer/HMAC; health action (GET ?action=health) is public',
    rateLimited: false,
    rateLimit: 'None',
    corsEnabled: true,
    isPublic: false,
