...
// Appelle la Edge Function verify-admin avec le token HMAC stocké en session.
// Retourne false si le token est absent, expiré ou invalide.
async function verifyAdminServerSide(): Promise<boolean> {
  try {
    ...
    // Compatibilité avec l'ancien format (pas de token HMAC)
    // Si la session ne contient pas de token signé, on rejette.
    if (!token) {
      console.warn("[Admin] Session sans token HMAC — connexion requise.");
      return false;
    }
    ...
    if (error || !data?.valid) {
      console.warn("[Admin] Vérification serveur échouée :", error?.message ?? data?.error);
      return false;
    }
...
export default function Admin() {
  ...
  // ── Génération de code ──────────────────────────────────────────────────────
  // Re-vérifie le token HMAC côté serveur avant chaque génération.
  ...
  // ── Suppression de code ─────────────────────────────────────────────────────
  // Re-vérifie le token HMAC côté serveur avant chaque suppression.
